...
\autor{Papp Tamás}
\autorEN{Papp Tamás}
\naslov{Fractal rendering with hybrid parallelism}
...
\oblast{High-Performance Computer Systems}

\mentor{Dr Veljko Petrović}
\mentorEN{Dr Veljko Petrović, PhD}

% Scientific field/discipline are standardized per study programme
% (Computing and Control), not per thesis topic -- values matched to
% another thesis from the same programme.
...
\izvod{Изложити теоретске основе за, и онда имплементирати механизам који
...
\abstractEN{Present the theoretical foundations for, and then implement, a
...
% Chapters, pages, references, tables, figures, graphs, appendixes.
% Pages = last numbered page before КЉУЧНА ДОКУМЕНТАЦИЈСКА ИНФОРМАЦИЈА
% (Introduction, page 1, through the end of the Biography); references =
% number of BibTeX entries in diplomski.bbl; figures = number of
% \begin{figure} blocks; tables/graphs/appendixes = 0, this thesis has
% none as separate elements.
\fizickiopis{8}{49}{21}{0}{29}{0}{0}
...
% Cover page: native, English-language typesetting (the class's
% \maketitle), since this is the English version of the thesis -- unlike
% the assignment sheet and declaration below, the cover isn't a
% university-issued form, so there's no reason to use the Serbian scan
% here.
\maketitle

% Official university forms (Q2_NA_04-02, 04-03): these bureaucratic
% forms are issued by the university only in Serbian, regardless of the
% thesis's own language, so no English-language version exists to insert
% here -- assignment sheet (04-03), no-conflict-of-interest declaration
% (04-02).
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \begin{tikzpicture}[x=5cm,y=5cm]
      \input{figures/sierpinski-triangle.tex}
    \end{tikzpicture}
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \begin{tikzpicture}[x=5cm,y=5cm]
      \input{figures/sierpinski-carpet.tex}
    \end{tikzpicture}
...
Listing~\ref{lst:mandelbrot-scalar} shows this kernel: the closed form test call right at the
start, then the iteration loop with the Brent style cycle detection described above.

\noindent\begin{minipage}{\linewidth}
\lstinputlisting[language=Rust, numbers=none,
  caption={Scalar \kod{f64} iteration of the Mandelbrot set, the reference
  implementation the SIMD, CUDA and WGSL kernels are algebraically derived from
...
Listing~\ref{lst:mandelbrot-simd} shows the \kod{active} mask from the \kod{f32x8} kernel
described above: the \kod{just\_escaped} check records which lanes just escaped via a bitwise
AND, and the immediately following \kod{active} update clears those lanes from the mask so
further updates no longer touch them, even though the whole vector keeps computing until the loop
ends.

\noindent\begin{minipage}{\linewidth}
\lstinputlisting[language=Rust, numbers=none,
  caption={Active lane mask in the \kod{f32x8} Mandelbrot kernel
  (\kod{src/fractal/kernels/mandelbrot.rs}).},
...
Listing~\ref{lst:perturb-rebase} shows \kod{perturb\_mandelbrot\_rebase}, the rebasing variant.
The \kod{er}/\kod{ei} update is a direct translation of the recurrence from
Equation~\ref{eq:perturbation}, and the check \kod{if zn\_sq < er * er + ei * ei || m ==
orbit.len} is the rebasing condition itself: once the squared modulus of the current orbit
position drops below the squared modulus of the delta itself, or the reference orbit runs out, the
current position $(az, bz)$ becomes the new delta and $m$ resets to zero.

\noindent\begin{minipage}{\linewidth}
\lstinputlisting[language=Rust, numbers=none,
  caption={Perturbation iteration with rebasing
...
Listing~\ref{lst:period3-before} is the test call before the fix, inside the \kod{mandelbrot\_f32}
kernel: \kod{cr}/\kod{ci} are widened to \kod{double} for the sole purpose of calling the
\kod{f64} version of the test. Listing~\ref{lst:period3-after} is the function that replaced it at
the call site, arithmetically identical except for staying in \kod{float} from start to finish.

\noindent\begin{minipage}{\linewidth}
\lstinputlisting[language=C++, numbers=none,
  caption={Period 3 test call before the fix, inside \kod{mandelbrot\_f32}:
  the check runs in \kod{double} inside an otherwise \kod{float} kernel.},
  label={lst:period3-before}]{listings/period3_before.cu}
\end{minipage}

\noindent\begin{minipage}{\linewidth}
\lstinputlisting[language=C++, numbers=none,
  caption={\kod{in\_period3\_bulb\_f32}, the \kod{float} version of the same test that
  replaced the call above (\kod{src/gpu/cuda/fractal.cu}).},
...
Listing~\ref{lst:corner-spread} is the pair of functions behind Equation~\ref{eq:corner-spread}.
\kod{sample\_corners} is the only pixel by pixel work the classifier does at all, exactly four
kernel calls per cell or sub-tile.

\noindent\begin{minipage}{\linewidth}
\lstinputlisting[language=Rust, numbers=none,
  caption={Corner sampling and normalized spread
...
Listing~\ref{lst:pi-controller} is the entire controller, some twenty lines. The
\kod{self.\allowbreak integral} update computes the moving average over \kod{history}, a circular
buffer of the last $16$ errors, and the last two lines of \kod{update}'s body are directly
Equation~\ref{eq:pi-controller}, clamp included.

\noindent\begin{minipage}{\linewidth}
\lstinputlisting[language=Rust, numbers=none,
  caption={PI controller for the classifier threshold
...
%% =====================================================================
%%  Key documentation information (required appendix) -- inserted as the
%%  official filled PDF (Q2_NA_04-05, Serbian KDI + English KWD pages)
%%  instead of the class's native \KDIpage/\KWDpage typesetting.
%% =====================================================================
